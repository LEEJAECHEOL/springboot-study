...
public class PostController {
	...
	public User testPostOcject(User user) { // x-www-form-urlencoded
		return user;
	}
	
	// @RequestBody을 사용하면 json 데이터를 받을 수 있음.
	@PostMapping("/test/post/json")
	public User testPostJson(@RequestBody User user) { // json을 전송
		return user; // MessageConverte가 낚아챔 -> json으로 변환해서 리턴 (RestController 일때 만)
	}
	
	// @Pathvariable
	@GetMapping("/test/post/{id}") // test/post/1
